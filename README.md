...
|---|---|
| **Plans actions** | `decision_node` evaluates all gathered evidence and selects one of two resolution paths |
| **Retrieves > 1 time** | First retrieval: incident history; Second retrieval: targeted SLA contract document after RCA |
| **Calls specialized tools** | 5 named tool calls: `query_incidents`, `get_sla_document`, `check_inventory`, `find_available_crews`, `escalate_to_vendor` |
| **Makes branching decisions** | Conditional LangGraph edge: `dispatch` path vs. `escalate` path based on SLA risk + recurrence |
| **Document-grounded output** | Every decision cites a specific contract excerpt (e.g., Clause 7.3, Clause 9.1) |
| **Enterprise-ready output** | Signed, timestamped Priority Action Report with full audit trail for NOC supervisor review |
...
flowchart TD
    ...
    Triage["🔎 Node 1: Triage<br/>Classify P0/P1/P2 Severity<br/>keyword-based log analysis"] --> Retrieve1

    Retrieve1["📂 Node 2: Retrieve Incidents<br/><b>TOOL:</b> query_incidents(tower_id)<br/>→ Incident Management System"]:::retrieval --> RCA

    RCA["🧠 Node 3: Root Cause Analysis<br/>Pattern match + symptom keywords<br/>→ Predicted Cause + Confidence %"] --> Retrieve2

    Retrieve2["📄 Node 4: Retrieve SLA Document<br/><b>TOOL:</b> get_sla_document(root_cause)<br/>→ Contract Database"]:::retrieval --> Decision
    ...
    Dispatch["🚛 Node 6A: Dispatch Path<br/><b>TOOL:</b> check_inventory(part)<br/><b>TOOL:</b> find_available_crews(cert)"]:::dispatch --> Report
    ...
    Report["📋 Node 7: Report Generation<br/>Signed Priority Action Report<br/>with cited documents + audit trail"]:::report --> End([✅ NOC Review])
```
